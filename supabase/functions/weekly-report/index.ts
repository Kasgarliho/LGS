...
Deno.serve(async (req) => {
  ...
  try {
    // DÜZELTME: Gizli anahtar isimleri güncellendi
    ...
    for (const koc of koclar) {
      ...
      for (const ogrenci of ogrenciler) {
        ...
        const { data: sorular, error: soruError } = await supabaseClient
          ...
          .eq('kullanici_id', ogrenci.baglanan_kullanici_id).gte('eklenme_zamani', birHaftaOnce.toISOString());
        ...
        await resend.emails.send({
          from: 'LGS Asistanı <onboarding@resend.dev>',
          to: koc.eposta,

...
export default function QuestionSolver({ questions, subjects, onFinish, onClose }: QuestionSolverProps) {
  ...
  const handleOpenModal = (isCorrect: boolean) => {
    const stat: SolvedStat = { subjectId: currentQuestion.subjectId, topic: currentQuestion.topic, correct: isCorrect };
    setSolvedStats(prev => [...prev, stat]);
    if (isCorrect) playYaySound(isMuted);
    else playFailSound(isMuted);
    setShowResult(true);
    ...
    return (
      <div className="fixed inset-0 bg-background/95 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <Card className="w-full max-w-2xl shadow-elegant animate-bounce-in">
          <CardHeader className="text-center"><CardTitle className="text-3xl font-bold gradient-primary bg-clip-text text-transparent">Quiz Tamamlandı! 🎉</CardTitle></CardHeader>
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center p-4 bg-success/10 rounded-lg"><p className="text-3xl font-bold text-success">{correctCount}</p><p className="text-sm text-muted-foreground">Doğru Cevap</p></div>
              <div className="text-center p-4 bg-destructive/10 rounded-lg"><p className="text-3xl font-bold text-destructive">{questions.length - correctCount}</p><p className="text-sm text-muted-foreground">Yanlış Cevap</p></div>
              <div className="text-center p-4 bg-warning/10 rounded-lg"><p className="text-3xl font-bold text-warning">%{accuracy}</p><p className="text-sm text-muted-foreground">Başarı Oranı</p></div>
            </div>
            <div className="text-center space-y-2">
              {accuracy >= 80 && (<Badge variant="outline" className="gradient-success text-success-foreground border-0 text-lg px-4 py-2">Mükemmel! 🏆</Badge>)}
              {accuracy >= 60 && accuracy < 80 && (<Badge variant="outline" className="gradient-warning text-warning-foreground border-0 text-lg px-4 py-2">İyi iş! 👏</Badge>)}
              {accuracy < 60 && (<Badge variant="outline" className="bg-secondary text-secondary-foreground text-lg px-4 py-2">Daha çok çalışmalısın! 💪</Badge>)}
            </div>
            <Button variant="hero" size="lg" className="w-full" onClick={handleFinish}>Devam Et</Button>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="fixed inset-0 bg-background/95 backdrop-blur-sm z-50 flex items-start justify-center p-4 md:pt-12">
      <Card className="w-full max-w-4xl shadow-elegant animate-slide-up">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              ...
              <CardTitle className="text-xl">Soru {currentQuestionIndex + 1} / {questions.length}</CardTitle>
            </div>
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2"><Clock className={`h-5 w-5 ${timeLeft <= 10 ? 'text-destructive animate-pulse' : 'text-warning'}`} /><span className={`font-bold ${timeLeft <= 10 ? 'text-destructive' : 'text-warning'}`}>{timeLeft}s</span></div>
        ...
        <CardContent className="space-y-6 px-6 py-4 max-h-[75vh] overflow-y-auto">
          <div className="bg-muted/30 p-4 rounded-lg"><h3 className="text-lg font-semibold text-foreground mb-4">{currentQuestion.question}</h3></div>
          <div className="grid gap-3">
            {currentQuestion.options.map((option, index) => {
              let buttonVariant: "outline" | "success" | "destructive" = "outline";
              let extraClasses = "";
              if (showResult) {
                if (index === currentQuestion.correctAnswer) { buttonVariant = "success"; extraClasses = "animate-pulse-glow"; } 
                else if (index === selectedAnswer && selectedAnswer !== currentQuestion.correctAnswer) { buttonVariant = "destructive"; }
              } else if (selectedAnswer === index) { extraClasses = "ring-2 ring-primary ring-offset-2"; }
              return (
